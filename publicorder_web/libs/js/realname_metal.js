		...
		var _v = {
						...
						common.dialog({
							...
							confirm: {
								event: function(template, hide) {
									...
									var _type = template.find('.ds_type').val();
									var _weight = template.find('.ds_weight').val();
									...
									_a.checkStuffInfoReturn({
										...
										function upDataWidthOutImage() {
											_a.createMetalInfo(personinfo, function() {
												var setting = {
			...
			_t = {
				searchResultsList: function(data) {
					...
					html = $(html);
					html.click(function() {
						_f.getmetalinfo(data.ds_shimingzhiid);
					});
					return html;
				},
				creatmetalpersoninfo: function(data) { //添加金属加工实名制信息dialog模板
					var creatinfo = '';
					creatinfo += '<div>'
					...
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>加工时间：</p><input type="text" value="" class="ds_date laydate-icon" style="width:448px; cursor:pointer;" onclick="laydate()">'
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>类别：</p><input type="text" value=""  placeholder="请输入类别" class="ds_type">'
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>重量：</p><input type="text" value=""  placeholder="请输入重量" class="ds_weight">'
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>物品描述：</p><textarea name="" placeholder="请输入物品描述" id="" cols="30" rows="10" class="ds_information"></textarea>'
					creatinfo += '</li>'
					creatinfo += '<li>'
				...
				creatmetalpersoninfo2: function(data) { //获取金属加工实名制信息dialog模板
					var creatinfo = '';
					creatinfo += '<div>'
					creatinfo += '<ul class="metalinfo">'
					creatinfo += '<li>'
					...
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>加工时间：</p><input type="text" value="' + (data['intoDate'] ? data['intoDate'] : '') + '" class="ds_date laydate-icon" style="width:448px; cursor:pointer;" onclick="laydate()" readonly="readonly"/>'
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>类别：</p><input type="text" value="' + (data['ds_type'] ? data['ds_type'] : '') + '"   class="ds_type" readonly="readonly">'
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>重量：</p><input type="text" value="' + (data['ds_weight'] ? data['ds_weight'] : '') + '"   class="ds_weight" readonly="readonly">'
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>物品描述：</p><textarea name="" id="" cols="30" rows="10" class="ds_information" readonly="readonly">' + (data['ds_information'] ? data['ds_information'] : '') + '</textarea>'
					creatinfo += '</li>'
					creatinfo += '<li>'
					creatinfo += '<p>上传物品照片：</p>'
				...
				search: function() {
					...
					html += '		<input type="radio" style="display:inline-block;float:none;width:35px; height:15px;border:1px solid #fff;" name="searchDate" value="2" class="all" > <span style="display:inline-block; line-height:27px;">全部</span> ';
					...
					html += '		<input type="radio" style="display:inline-block;float:none;width:35px; height:15px; border:1px solid #fff;" name="searchDate" value="0" class="threeMonth" checked="checked"> <span style="display:inline-block; line-height:27px;">近三个月</span> ';
					html += '	</div>';
					html = $(html);
					// return html; // 临时更改
					var datatype = 0;
					getSearchResult = function() {
						...
						var option = {
							element: '#paging2',
							data: {
								// text: html.find('.searchResult').val(),
							...
							success: function(result) {
								if (result.list.length == 0) {
									...
									//$('#paging4').css('display', 'block');
									$('.realnamebody .listbox').empty();
									//$('.realnamebody .customlistbox').css('display', 'none');
			...
			_a = {
				...
				/*添加金属加工实名制信息*/
				createMetalInfo: function(data, success) {
					common.POST({
						data: data,
						url: "/crm/Entities",
						success: function(result) {
